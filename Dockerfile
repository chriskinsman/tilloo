...
RUN apk update && apk upgrade && \
    apk add --no-cache bash git openssh
# install node packages
RUN cd /tilloo && npm ci --only=production && npm i https://github.com/chriskinsman/node-tail && cd app/public && npm ci --only=production && cd /tilloo/web/client && npm ci --only=production

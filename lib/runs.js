...
console.info('Runs connecting to %s:%d', config.disque.host, config.disque.port);
    ...
    Run.findByRunId(runId, function(err, run) {
        if(err) {
            ...
            async.parallel([
                ...
                function(done) {
                    if(force) {
                        disq.addJob({ queue: constants.QUEUES.STATUS, job: JSON.stringify({ status: constants.JOBSTATUS.FAIL, runId: run._id, type:constants.KILLTYPE.MANUAL, source: 'runs' }), timeout: 0 }, function(err) {
                                if(err) {

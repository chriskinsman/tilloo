...
exports.sendEmail = function( message, callback ){
    ...
    function sendMail( jobId, txt ){
        ...
        Job.findByJobId(jobId,function(error,job){
            ...
            var html = '<b>'+txt+'</b> <a href="http://'+config.web.host+':'+config.web.port+'/job/'+jobId+'">Click here to view job.</a>';

            if(!error) {
                html = '<b>' + txt + '</b> <a href="http://' + config.web.host + ':' + config.web.port + '/job/' + jobId + '">View job: ' + job.name + '.</a>';
            }

...
async function main(initialResourceVerison) {
    let jobStream;
    ...
    async function initializeStream() {
        ...
        jobStream = await k8sClient.apis.batch.v1.watch.namespaces(constants.NAMESPACE).jobs.getObjectStream({ qs: { resourceVersion: lastResourceVersion } });
        jobStream.on('data', async (jobData) => {
            ...
            if (jobData.object.metadata && jobData.object.metadata.resourceVersion) {
                const currentResourceVersion = parseInt(jobData.object.metadata.resourceVersion, 10);
                if (currentResourceVersion > lastResourceVersion) {
                    ...
                    debug(`skipping old event lastResourceVersion: ${lastResourceVersion}, resourceVersion: ${jobData.object.metadata.resourceVersion}`, jobData.object.metadata);
                }
            }
        });

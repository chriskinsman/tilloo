...
async function main(initialResourceVersion) {
    let eventStream;
    ...
    async function initializeStream() {
        ...
        debug(`${_id} Initializing eventStream resourceVersion: ${lastResourceVersion}`);
        eventStream = await k8sClient.api.v1.watch.namespaces(constants.NAMESPACE).events.getObjectStream({ qs: { resourceVersion: lastResourceVersion } });
        eventStream.on('data', async (eventData) => {
            ...
            if (eventData.object.metadata && eventData.object.metadata.resourceVersion) {
                const currentResourceVersion = parseInt(eventData.object.metadata.resourceVersion, 10);
                if (currentResourceVersion > lastResourceVersion) {
                    ...
                    try {
                        ...
                        debug(`${_id} Updated lastResourceVersion`, currentResourceVersion);
                    }
                    catch (err) {
                    ...
                    debug(`${_id} skipping old event lastResourceVersion: ${lastResourceVersion}, resourceVersion: ${eventData.object.metadata.resourceVersion}`, eventData.object.metadata);
                }
            }
        });
...
(async () => {
    ...
    try {
        ...
        await main(checkpoint.resourceVersion);
    }
    catch (err) {
